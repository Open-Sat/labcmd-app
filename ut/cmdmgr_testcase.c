...
void UtSetExpectedEvent(uint16 EventID, uint16 EventType, boolean DisplayEvent)
{
...
int32 UtSendEventHook(uint16 EventID, uint16 EventType, char *EventText)
{
...
**
*/
void UtInitCmdPkt(CFE_SB_MsgPtr_t MsgPtr, CFE_SB_MsgId_t MsgId, uint16 Length, uint8 FuncCode)
{
...
boolean UtValidateChecksum(CFE_SB_MsgPtr_t MsgPtr)
{
...
uint16 UtGetUserDataLength(CFE_SB_MsgPtr_t MsgPtr)
{
...
uint16 UtGetCmdCode(CFE_SB_MsgPtr_t MsgPtr)
{
...
uint16 UtGetChecksum(CFE_SB_MsgPtr_t MsgPtr)
{
...
void CMDMGR_Test02(void)
{
   ...
   */
   ...
   for (i=0; i < CMDMGR_CMD_FUNC_TOTAL; i++)
   {
      ...
      {
         /* This assumes the no param FC is 0 becuase it clobbers the command packet */
