...
**
*/
boolean UPLINK_ConfigMsgTunnelCmd(const CFE_SB_MsgPtr_t MsgPtr)
{
...
**
*/
void UPLINK_Constructor(UPLINK_Class*  UplinkPtr)
{
   ...
   if ( (Uplink->SocketId = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) >= 0)
   {
      ...
      Uplink->SocketAddress.sin_port        = htons(LABCMD_PORT);
...
int UPLINK_Read(uint16 MaxMsgRead)
{
    ...
    for (i = 0; i < MaxMsgRead; i++)
    {
        ...
        Status = recvfrom(Uplink->SocketId, (char *)&(Uplink->RecvBuff[0]), sizeof(Uplink->RecvBuff), MSG_DONTWAIT,
        ...
        else
        {
            ...
            {
            	OS_printf("LABCMD: Read %d bytes from socket\n",Status);
                Uplink->RecvMsgCnt++;
                ...
                CFE_SB_SendMsg(Uplink->RecvMsgPtr);
            }
            else
            {
...
**
** This function is called when the labcmd app is terminated. This should
** never occur but if it does this will close the network socket.
*/
static void DestructorCallback(void)
{

   OS_printf("LABCMD delete callback. Closing Network socket.\n");
